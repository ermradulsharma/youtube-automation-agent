...
const inquirerRaw = require('inquirer');
const baseInquirer = inquirerRaw.default || inquirerRaw;
const inquirer = {
  ...baseInquirer,
  prompt: (questions, answers) => {
    const normalize = (q) => (q && q.type === 'list' ? { ...q, type: 'select' } : q);
    const normalizedQuestions = Array.isArray(questions) ? questions.map(normalize) : normalize(questions);
    return baseInquirer.prompt(normalizedQuestions, answers);
  }
};
...
class CredentialManager {
  ...
  async validateAll() {
    ...
    if (!this.tokens.youtube) {
      console.log(chalk.cyan('ℹ️  YouTube upload token not connected yet — local video generation is ACTIVE (videos save to data/videos/).'));
    }
