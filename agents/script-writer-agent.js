...
class ScriptWriterAgent {
  ...
  async generateScript(rawStrategy) {
    try {
      const strategy = {
        topic: 'YouTube Automation with AI',
        angle: 'Step-by-step practical guide and demonstration',
        targetAudience: 'General audience interested in educational content',
        contentType: 'Explainer',
        keywords: ['youtube', 'automation', 'ai'],
        ...(rawStrategy || {})
      };
      this.logger.info(`Generating script for: ${strategy.topic}`);
      
      const contentTypeKey = String(strategy.contentType || 'explainer').toLowerCase();
      const template = this.templates[contentTypeKey] || this.templates.explainer;
...
Topic: ${strategy.topic || 'YouTube Automation'}
Style/content type: ${strategy.contentType || 'Explainer'}
Angle: ${strategy.angle || 'Step-by-step practical guide'}
...
Avoid fabricated statistics, unsupported claims, and fake urgency. List every externally verifiable factual claim in claims. Use only exact URLs from Research sources; use an empty sourceUrls array when the supplied sources do not support a claim.`;
  ...
  async generateTitle(strategy) {
    const topic = String(strategy?.topic || 'YouTube Automation').trim();
    const angle = String(strategy?.angle || 'Practical Guide').trim();
    const contentType = String(strategy?.contentType || 'Explainer').trim();

    const templates = [
      `${angle}`,
      `${topic}: The Complete Guide`,
      `Everything You Need to Know About ${topic}`,
      `${topic} in ${new Date().getFullYear()}: What's Changed?`,
      `The Truth About ${topic} (Shocking Results)`,
      `How to Master ${topic} in 30 Days`,
      `${topic}: Beginner to Expert Guide`
    ];

    if (contentType === 'Tutorial') {
      return `How to ${topic}: Step-by-Step Guide`;
    } else if (contentType === 'List') {
      return `Top 10 ${topic} Tips You Need to Know`;
    } else if (contentType === 'Review') {
      return `${topic} Review: Is It Worth It?`;
    }
