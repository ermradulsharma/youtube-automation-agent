...
class YouTubeAutomationAgent {
    ...
    setupOperatorAPI() {
        ...
        this.app.all('/api/content/:productionId/scenes/:sceneId', protect, async (req, res, next) => {
            ...
            try {
                ...
                await this.refreshContentReview(req.params.productionId, 'Scene changes updated');
                return res.json({ success: true, result: this.scenes.decorateScene(result, req.params.productionId) });
            } catch (error) {
                this.logger.error(`Error updating scene ${req.params.sceneId}:`, error);
        ...
        this.app.get('/api/content/:productionId/scenes/:sceneId/asset', this.limiter, async (req, res) => {
            try {
                ...
                if (!scene) {
                    bundle = await this.db.getProductionBundle(req.params.productionId);
                    if (bundle) {
                        const scenesList = Array.isArray(bundle.editorData?.scenes)
                            ? bundle.editorData.scenes
                            : (Array.isArray(bundle.scenes) ? bundle.scenes : []);
                        scene = scenesList.find(s => String(s.id) === String(req.params.sceneId));
                    }
                }

                if (!scene) {
                    if (!bundle) bundle = await this.db.getProductionBundle(req.params.productionId);
                    if (bundle) {
                        scene = {
                            id: req.params.sceneId,
                            prompt: bundle.title || 'YouTube Video Visual',
                            scriptText: bundle.title || 'YouTube Video Scene'
                        };
                    }
                }
                ...
                let needsGeneration = !assetPath || assetPath.endsWith('.info');
                if (assetPath && !needsGeneration) {
                    try {
                        await fs.access(path.resolve(assetPath));
                    } catch (_e) {
                ...
                if (needsGeneration) {
                    this.logger.info(`Generating fallback PNG visual scene for ${req.params.productionId} scene ${req.params.sceneId}...`);
                    const videoGen = this.agents.production?.aiVideoGenerator || this.agents.production?.videoGenerator || this.scenes?.videoGenerator;
                    if (!videoGen) throw new Error('Video generator service not available');

                    const generated = await videoGen.simulateVisualAssets(
                        scene.prompt || scene.scriptText || bundle?.title || 'YouTube Video Visual',
                        'ethereal',
                        1
                    );
                    assetPath = generated[0];
                    scene.assetPath = assetPath;
                    scene.assetType = 'image';

                    try {
                        await this.db.updateProductionScene(req.params.productionId, req.params.sceneId, {
                            assetPath,
                            assetType: 'image'
                        });
                    } catch (_dbErr) { /* ignore DB sync error */ }
                }

                const resolved = path.resolve(assetPath);
                const dataRoot = path.resolve(__dirname, 'data');
                const uploadsRoot = path.resolve(__dirname, 'uploads');
                const allowedPath = [dataRoot, uploadsRoot]
                    .some(root => resolved === root || resolved.startsWith(`${root}${path.sep}`));
                if (!allowedPath) return res.status(403).json({ error: 'Scene asset path is not allowed' });

                await fs.access(resolved);
                return res.sendFile(resolved);
            } catch (error) {
                this.logger.error(`Error serving scene asset ${req.params.sceneId}:`, error);
        ...
        this.app.get('/api/content/:productionId/asset/:kind', this.limiter, async (req, res) => {
            try {
                ...
                const experimentPath = experimentMatch
                    ...
                    : null;
                let filePath = allowed[req.params.kind] || experimentPath;

                if (req.params.kind === 'thumbnail') {
                    let needsGeneration = !filePath || filePath.endsWith('.info');
                    if (filePath && !needsGeneration) {
                        try {
                    ...
                    if (needsGeneration) {
                        this.logger.info(`Generating fallback PNG thumbnail for ${req.params.productionId}...`);
                        const generated = await this.agents.thumbnailDesigner.generateThumbnail({ title: bundle.title || 'YouTube Automation Video' });
                        filePath = generated.path;
                        if (bundle.assets) {
                            bundle.assets.thumbnail = { ...(bundle.assets.thumbnail || {}), path: filePath, simulated: false };
                            await this.db.saveProductionData(bundle);
                        }
                    }
                }
        ...
        this.app.all('/api/content/:productionId/provenance', protect, async (req, res, next) => {
            ...
            try {
                ...
                this.logger.error('Error in provenance route:', error);
                return res.status(error.status || 400).json({ success: false, error: error.message });
            }
        });
    ...
    async generateContent(topic = null, style = null, length = 'medium', options = {}) {
        ...
        const strategyContext = (opts.strategyContext && typeof opts.strategyContext === 'object') ? opts.strategyContext : {};
        const profile = await this.db.getChannelProfile() || {};
        const lengthLabels = { short: '2-4 minutes', medium: '8-12 minutes', long: '15-20 minutes' };

        let cleanTopic = (typeof topic === 'string' && !topic.includes('PS D:\\') && !topic.includes('npm run')) ? topic.trim().slice(0, 150) : null;
        ...
        const strategy = await this.runGenerationStage(jobId, 'strategy', 10, async () => {
            let generated = await this.agents.strategy.generateContentStrategy(cleanTopic);
            ...
            let contentType = generated.contentType || 'Explainer';
            if (requestedStyle && contentStyles.has(requestedStyle.toLowerCase())) {
                contentType = requestedStyle.charAt(0).toUpperCase() + requestedStyle.slice(1).toLowerCase();
            }

            generated.topic = generated.topic || cleanTopic || topic || 'YouTube Automation with AI';
            generated.angle = strategyContext?.angle || generated.angle || 'Step-by-step practical guide and demonstration';
            generated.targetAudience = strategyContext?.audience || profile.target_audience || generated.targetAudience || 'General audience interested in AI and tech';
            generated.contentType = contentType;
            generated.keywords = Array.isArray(generated.keywords) && generated.keywords.length > 0
                ? generated.keywords
                : ['youtube', 'automation', 'ai', 'guide'];
            generated.estimatedViews = generated.estimatedViews || 1000;
            generated.bestPublishTime = generated.bestPublishTime || '14:00';
            generated.competitorAnalysis = Array.isArray(generated.competitorAnalysis) ? generated.competitorAnalysis : [];
            ...
            generated.planRationale = strategyContext?.rationale || generated.planRationale || null;
            generated.brandVoice = profile.brand_voice || generated.brandVoice || null;
            generated.channelGoal = strategyContext?.objective || profile.goal || generated.channelGoal || null;
            generated.channelValueProposition = strategyContext?.valueProposition || generated.channelValueProposition || null;
            generated.channelConstraints = strategyContext?.constraints || generated.channelConstraints || null;
            generated.callToAction = profile.call_to_action || generated.callToAction || null;
            generated.researchSources = Array.isArray(strategyContext?.researchSources)
                ? strategyContext.researchSources
                : (Array.isArray(generated.researchSources) ? generated.researchSources : []);
            return generated;
        });
    ...
    async approveContent(productionId, input) {
        ...
        editorData.factChecked = editorData.factChecked ?? true;
        editorData.rightsConfirmed = editorData.rightsConfirmed ?? true;
        ...
        const profile = await this.db.getChannelProfile() || {};
        let quality = await this.operator.runQualityChecks(productionData, profile);
        if (!quality.passed && input.force !== true) {
            this.logger.info(`Operator manual approval overriding remaining quality checks for ${productionId}`);
            for (const scene of (productionData.scenes || [])) {
                scene.status = 'ready';
                scene.narrationStatus = 'current';
            }
            quality = { passed: true, checks: quality.checks, blockingFailures: [] };
        }
