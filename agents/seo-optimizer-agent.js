...
class SEOOptimizerAgent {
  ...
  async optimize(script, rawStrategy) {
    try {
      const strategy = {
      ...
      const keywordsList = Array.isArray(strategy.keywords) ? strategy.keywords : [];
      const seoData = {
        title,
        description,
        tags,
        ...
        seoScore,
        metadata: {
          primaryKeyword: keywordsList[0] || 'guide',
          secondaryKeywords: keywordsList.slice(1, 5),
...
Keep tags under YouTube's 500 character total guidance. Avoid fabricated statistics and unsupported claims.`;
  ...
  async generateDescription(script, strategy) {
    ...
    const angleText = String(strategy?.angle || strategy?.topic || '').toLowerCase();
  ...
  async generateHashtags(strategy) {
    ...
    if (strategy?.contentType) {
      hashtags.push(`#${String(strategy.contentType).toLowerCase()}`);
    }
