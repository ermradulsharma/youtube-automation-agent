    ...
    async runQualityChecks(production, profile = {}) {
        ...
        if (topic) {
            const duplicates = await this.db.getRow(
                `SELECT COUNT(*) AS count FROM publish_schedule
         ...
         AND published_at >= datetime('now', '-90 days')`,
            ...
            checks.push(this.check('duplicate_topic', unique,
                unique
                    ? 'No duplicate published topic detected in the last 90 days'
                    : 'This exact topic was already published recently', false));
        }
