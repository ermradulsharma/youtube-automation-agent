...
class AITextService {
  ...
  async generateText(prompt, options = {}) {
    ...
    if (this.gemini) {
      const modelsToTry = Array.from(new Set([model, 'gemini-3.1-pro-preview', 'gemini-3.5-flash-lite', 'gemini-3.7-flash']));
      let lastError = null;

      for (const currentModel of modelsToTry) {
        try {
          const config = { maxOutputTokens: maxTokens };
          if (!/^gemini-3\.(?:[5-9]|\d{2,})-/.test(currentModel)) config.temperature = temperature;
          ...
          lastError = error;
          this.logger.warn(`Gemini model ${currentModel} returned error (${error.status || error.message || '503'}), trying fallback model...`);
          await new Promise(resolve => setTimeout(resolve, 500));
        }
      }

      throw lastError || new Error(`${this.providerName} returned empty response on all models.`);
    }
