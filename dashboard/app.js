...
function showToast(message, type = 'success', durationMs = 8000) {
  const toast = $('#toast');
  toast.textContent = message;
  toast.className = `toast ${type}`;
  clearTimeout(ui.toastTimer);
  const timeout = type === 'error' ? Math.max(durationMs, 12000) : durationMs;
  ui.toastTimer = setTimeout(() => toast.classList.add('hidden'), timeout);
}
  ...
  if (milestones.firstRealVideo?.achieved) {
    container.insertAdjacentHTML('beforeend', `
      <div class="activation-share">
        ...
        <a class="button secondary small" href="https://github.com/ermradulsharma/youtube-automation-agent/discussions/new?category=show-and-tell" target="_blank" rel="noreferrer">Share what you built</a>
      </div>`);
  }
}
