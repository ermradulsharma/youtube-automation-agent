...
class PublishingSchedulingAgent {
  ...
  async setupYouTubeAPI() {
    try {
      ...
      this.logger.warn(`YouTube upload API not connected: ${error.message}. Generated videos will save locally in data/videos/.`);
      this.youtube = null;
    }
  }
  ...
  async uploadToYouTube(scheduleEntry) {
    ...
    if (scheduleEntry.publishTime) {
      const pubDate = new Date(scheduleEntry.publishTime);
      const now = new Date();
      if (!isNaN(pubDate.getTime()) && pubDate.getTime() > now.getTime() + 60000) {
        statusObj.publishAt = pubDate.toISOString();
